...
(function () {
    const COUNT = 15;
    const container = document.querySelector(".contact .fireflies");
    if (!container) return;
    ...
    for (let i = 0; i < COUNT; i++) {
        ...
        const rotateDur = rand(28, 45).toFixed(2) + "s";
        const flashDur = rndInt(14000, 28000) + "ms";
        const flashDelay = rndInt(8000, 20000) + "ms";
        const travelDur = rndInt(400, 900) + "s";
        ...
        styleTag.sheet.insertRule(
            `.contact .firefly[data-i="${i}"]::before { animation-duration: var(--rotateDur); }`,
            styleTag.sheet.cssRules.length
        );

        styleTag.sheet.insertRule(
            `.contact .firefly[data-i="${i}"]::after { animation-duration: var(--rotateDur), var(--flashDur); animation-delay: 0ms, var(--flashDelay); }`,
            styleTag.sheet.cssRules.length
        );
