...
(function () {
  const circles = Array.from(
    document.querySelectorAll(".hobby-circles .circle")
  );
...
(function () {
  ...
  document.addEventListener("keydown", (e) => {
    if (
      ...
      successPanel.classList.contains("open")
    ) {
      hideContactSuccess();
    }
  });
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          response.status === 200 &&
          (json === null || json.success === true || json.message)
        ) {
          showContactSuccess();
          form.reset();
        } else {
          ...
          if (card) {
            card.querySelector("h3").textContent =
              "Thanks — but there was a hiccup";
            card.querySelector("p").textContent =
              "Your message was received by FormSubmit but the server returned a non-standard response. If you don’t get an email, please try again later.";
          }
          showContactSuccess();
        }
      } else {
